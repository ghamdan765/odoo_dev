...
class EstatePropertyType(models.Model):
    ...
    # Basic
    name = fields.Char(string='Name', required=True, help="The type name of property.")
    sequence = fields.Integer("Sequence", default=1,help="Used to order properties")

    # Relational (for inline view)
    property_ids = fields.One2many("estate.property", "proerty_type_id", string="Properties")
    ...
    def _compute_offer(self):
            for prop in self:
                prop.offer_count = len(prop.offer_ids) if prop.offer_ids else 0
